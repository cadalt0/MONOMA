...
export default function SignupPage() {
  ...
  const [isLoginBusy, setIsLoginBusy] = useState<boolean>(false)
  ...
  // No GSI render: we only use the popup flow via useGoogleLogin

  // No UI timeout needed for popup login
    ...
    const login = useGoogleLogin({
      ...
      onSuccess: async (resp) => {
        try {
          ...
          const accessToken = (resp as unknown as { access_token?: string }).access_token
          const idToken = (resp as unknown as { id_token?: string }).id_token
          if (accessToken) {
            try {
          ...
          setUserEmail(email)
          try { localStorage.setItem("monomaEmail", email) } catch {}
          try {
      ...
      onError: () => setIsLoginBusy(false),
      onNonOAuthError: () => setIsLoginBusy(false),
    })

    return (
      <button
        onClick={() => { if (!isLoginBusy) try { login() } catch {} }}
        disabled={isLoginBusy}
        className="inline-flex items-center gap-3 bg-white text-black font-medium rounded-full px-4 py-2 shadow hover:bg-white/90 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-black disabled:opacity-60"
      >
        ...
        <span className="text-sm">{isLoginBusy ? "Signing in..." : "Continue with Google"}</span>
      </button>
    )
  }
  ...
  return (
    <div className="relative flex min-h-screen w-full overflow-hidden">
      ...
      <div className="relative z-10 flex w-full items-center justify-center p-4 md:p-8">
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex justify-center">
                    {googleClientId ? (
                      ...
                      <button
                        disabled
                        className="inline-flex items-center gap-3 bg-white text-black font-medium rounded-full px-4 py-2 opacity-60 cursor-not-allowed"
                      >
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 48 48" className="w-5 h-5">
                          <path fill="#FFC107" d="M43.611 20.083H42V20H24v8h11.303C33.602 32.329 29.17 36 24 36c-6.627 0-12-5.373-12-12s5.373-12 12-12c3.059 0 5.84 1.154 7.961 3.039l5.657-5.657C33.64 6.053 29.065 4 24 4 16.318 4 9.656 8.337 6.306 14.691z"/>
                          <path fill="#FF3D00" d="M6.306 14.691l6.571 4.818C14.739 15.108 18.981 12 24 12c3.059 0 5.84 1.154 7.961 3.039l5.657-5.657C33.64 6.053 29.065 4 24 4 16.318 4 9.656 8.337 6.306 14.691z"/>
        ...
        {/* Removed notice since we now use direct popup login */}
      </div>
    </div>
  )
}
